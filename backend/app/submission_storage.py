...
class SubmissionStorage:
        ...
        try:
            with temporary.open("xb") as target:
                while chunk := await upload.read(64 * 1024):
                    ...
                    target.write(chunk)
            if size == 0:
                raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Source file is empty")
            temporary.replace(destination)
        finally:
            temporary.unlink(missing_ok=True)
            await upload.close()

        return StoredSource(original_filename, storage_key, size, digest.hexdigest())
    ...
    def _validate_filename(filename: str | None) -> str:
        ...
        if PurePosixPath(normalized).name != normalized:
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Filename must not contain a path"
            )
        sanitized = SAFE_FILENAME_PATTERN.sub("_", normalized).strip(".")
        if not sanitized or Path(sanitized).suffix.lower() != ".py":
            raise HTTPException(
                status_code=status.HTTP_415_UNSUPPORTED_MEDIA_TYPE, detail="Only .py files are accepted"
            )
