import io
import json
import re
import tarfile
import time
from contextlib import suppress
from dataclasses import dataclass
from pathlib import Path

import docker
from docker.errors import DockerException, NotFound
from docker.types import LogConfig
from requests import ConnectionError as RequestsConnectionError
from requests import ReadTimeout

from app.config import get_settings
from app.models import TestCase, TestOutcome

REPORT_PLUGIN = r"""
import json
import time
from pathlib import Path

started = time.monotonic()
results = []

def _detail(report):
    text = str(report.longrepr) if report.failed else ""
    return text[-4000:]

def pytest_runtest_logreport(report):
    if report.when == "call":
        kind = None
        if report.failed:
            kind = "assertion_failure" if "AssertionError" in _detail(report) else "runtime_error"
        results.append({"nodeid": report.nodeid, "outcome": report.outcome,
                        "duration_ms": round(report.duration * 1000),
                        "failure_type": kind, "failure_detail": _detail(report)})
    elif report.when == "setup" and report.failed:
        results.append({"nodeid": report.nodeid, "outcome": "error",
                        "duration_ms": round(report.duration * 1000),
                        "failure_type": "runtime_error", "failure_detail": _detail(report)})

def pytest_collectreport(report):
    if report.failed:
        detail = _detail(report)
        kind = "syntax_error" if "SyntaxError" in detail else "collection_error"
        results.append({"nodeid": report.nodeid, "outcome": "error", "duration_ms": 0,
                        "failure_type": kind, "failure_detail": detail})

def pytest_sessionfinish(session, exitstatus):
    payload = {"runtime_ms": round((time.monotonic() - started) * 1000), "results": results}
    Path("/workspace/grading-report.json").write_text(json.dumps(payload), encoding="utf-8")
"""


@dataclass(frozen=True)
class SandboxTestResult:
    test_case_id: int
    test_name: str
    outcome: TestOutcome
    duration_ms: int
    failure_type: str | None
    failure_detail: str | None


@dataclass(frozen=True)
class SandboxResult:
    timed_out: bool
    infrastructure_error: str | None
    runtime_ms: int
    output: str
    results: list[SandboxTestResult]


class DockerSandboxRunner:
    def __init__(self) -> None:
        self.settings = get_settings()

    def run(self, source_path: Path, test_cases: list[TestCase]) -> SandboxResult:
        if not source_path.is_file():
            return SandboxResult(False, "Submission source is unavailable", 0, "", [])
        container = None
        workspace_volume = None
        workspace_helper = None
        started = time.monotonic()
        try:
            client = docker.from_env(timeout=self.settings.grading_timeout_seconds + 5)
            workspace_volume = client.volumes.create(labels={"app": "grading-sandbox"})
            workspace_helper = client.containers.create(
                self.settings.grading_sandbox_image,
                ["chown", "10001:10001", "/workspace"],
                network_disabled=True,
                user="0:0",
                volumes={workspace_volume.name: {"bind": "/workspace", "mode": "rw"}},
            )
            workspace_helper.put_archive("/workspace", self._workspace_archive(source_path, test_cases))
            workspace_helper.start()
            workspace_helper.wait()
            workspace_helper.remove()
            workspace_helper = None
            container = client.containers.create(
                self.settings.grading_sandbox_image,
                ["python", "-m", "pytest", "-q", "-p", "grading_plugin", "-p", "no:cacheprovider", "tests"],
                network_disabled=True,
                mem_limit=self.settings.grading_memory,
                memswap_limit=self.settings.grading_memory,
                nano_cpus=int(self.settings.grading_cpus * 1_000_000_000),
                pids_limit=self.settings.grading_pids_limit,
                read_only=True,
                cap_drop=["ALL"],
                security_opt=["no-new-privileges"],
                user="10001:10001",
                tmpfs={"/tmp": "rw,noexec,nosuid,size=16m,mode=1777"},
                volumes={workspace_volume.name: {"bind": "/workspace", "mode": "rw"}},
                working_dir="/workspace",
                environment={"HOME": "/tmp", "PYTHONDONTWRITEBYTECODE": "1"},
                log_config=LogConfig(
                    type="json-file",
                    config={"max-size": "64k", "max-file": "1"},
                ),
            )
            container.start()
            try:
                container.wait(timeout=self.settings.grading_timeout_seconds)
            except (ReadTimeout, RequestsConnectionError) as exc:
                if "timed out" not in str(exc).lower():
                    raise
                with suppress(DockerException, RequestsConnectionError):
                    container.kill()
                return SandboxResult(True, None, self._elapsed(started), self._logs(container), [])
            output = self._logs(container)
            try:
                stream, _ = container.get_archive("/workspace/grading-report.json")
            except NotFound:
                return SandboxResult(
                    False, "Sandbox did not produce a valid test report", self._elapsed(started), output, []
                )
            report = self._archive_file(b"".join(stream))
            return self._parse_report(report, test_cases, output)
        except (DockerException, OSError, ValueError, json.JSONDecodeError) as exc:
            detail = f"Sandbox infrastructure failure ({type(exc).__name__}: {str(exc)[:500]})"
            return SandboxResult(False, detail, self._elapsed(started), "", [])
        finally:
            if container is not None:
                with suppress(DockerException):
                    container.remove(force=True)
            if workspace_helper is not None:
                with suppress(DockerException):
                    workspace_helper.remove(force=True)
            if workspace_volume is not None:
                with suppress(DockerException):
                    workspace_volume.remove(force=True)

    @staticmethod
    def _workspace_archive(source_path: Path, test_cases: list[TestCase]) -> bytes:
        buffer = io.BytesIO()
        with tarfile.open(fileobj=buffer, mode="w") as archive:
            DockerSandboxRunner._add_bytes(archive, "solution.py", source_path.read_bytes())
            DockerSandboxRunner._add_bytes(archive, "grading_plugin.py", REPORT_PLUGIN.encode())
            for test_case in test_cases:
                DockerSandboxRunner._add_bytes(
                    archive, f"tests/test_{test_case.id}.py", test_case.content.encode("utf-8")
                )
        return buffer.getvalue()

    @staticmethod
    def _add_bytes(archive: tarfile.TarFile, name: str, content: bytes) -> None:
        info = tarfile.TarInfo(name)
        info.size = len(content)
        info.mode = 0o444
        archive.addfile(info, io.BytesIO(content))

    @staticmethod
    def _archive_file(content: bytes) -> bytes:
        if len(content) > 1_048_576:
            raise ValueError("Sandbox report exceeds its limit")
        with tarfile.open(fileobj=io.BytesIO(content)) as archive:
            member = archive.next()
            extracted = archive.extractfile(member) if member else None
            if extracted is None:
                raise ValueError("Invalid sandbox report archive")
            return extracted.read(1_048_577)

    def _parse_report(self, content: bytes, test_cases: list[TestCase], output: str) -> SandboxResult:
        payload = json.loads(content.decode("utf-8"))
        by_id = {test_case.id: test_case for test_case in test_cases}
        results = []
        for item in payload.get("results", []):
            match = re.search(r"test_(\d+)\.py(?:::(.+))?", str(item.get("nodeid", "")))
            if not match or int(match.group(1)) not in by_id:
                continue
            test_case_id = int(match.group(1))
            value = item.get("outcome")
            outcome = (
                TestOutcome.PASSED
                if value == "passed"
                else (TestOutcome.FAILED if value == "failed" else TestOutcome.ERROR)
            )
            results.append(
                SandboxTestResult(
                    test_case_id,
                    match.group(2) or by_id[test_case_id].name,
                    outcome,
                    max(0, int(item.get("duration_ms", 0))),
                    item.get("failure_type"),
                    str(item.get("failure_detail") or "")[-4000:] or None,
                )
            )
        return SandboxResult(False, None, max(0, int(payload.get("runtime_ms", 0))), output, results)

    def _logs(self, container: object) -> str:
        content = container.logs(stdout=True, stderr=True, tail=200)
        return content[-self.settings.grading_output_max_bytes :].decode("utf-8", errors="replace")

    @staticmethod
    def _elapsed(started: float) -> int:
        return round((time.monotonic() - started) * 1000)
